...
_WATCHDOG_ACTIONS_WITH_ARGS = {"verify"}
...
def _match_prefixed(text: str) -> ParsedCommand | None:
    ...
    if bot_raw in {"tradebot", "tb"}:
        ...
        if action and args_raw:
            if action_raw in _WATCHDOG_ACTIONS_WITH_ARGS:
...
def source_for_action(action: str | None) -> str:
    ...
    if token.startswith("auditor"):
        return "Auditor"
    if token.startswith("watchdog") or token == "clearchat" or token.startswith("verify"):
        return "WatchDog"
