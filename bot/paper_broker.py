...
class RiskState:
    ...
    def from_dict(cls, data: dict | None) -> "RiskState":
        ...
        # Prune stale paused_until: clear it when the pause window has already elapsed
        # so we don't start in a spurious hibernation after a long absence.
        paused_until_raw: str | None = data.get("paused_until")
        if paused_until_raw:
            try:
                if now >= datetime.fromisoformat(paused_until_raw):
                    paused_until_raw = None
            except (ValueError, TypeError):
                paused_until_raw = None

        # Prune stale hour window: reset trades_this_hour when the recorded window
        # started more than 1h ago so the limiter doesn't carry over across restarts.
        hour_window_start_raw: str | None = data.get("hour_window_start")
        trades_this_hour_val: int = int(data.get("trades_this_hour", 0))
        if hour_window_start_raw:
            try:
                if (now - datetime.fromisoformat(hour_window_start_raw)).total_seconds() >= 3600:
                    hour_window_start_raw = None
                    trades_this_hour_val = 0
            except (ValueError, TypeError):
                hour_window_start_raw = None
                trades_this_hour_val = 0
        ...
        return cls(
            ...
            trades_this_hour=trades_this_hour_val,
