...
from bot.auditor.proposer import ALLOWED_KNOBS, propose_changes
...
class AuditorService:
    ...
    def confirm_proposal(self, proposal_id: str) -> str:
        ...
        if not proposal_id:
            return "Provide a proposal ID — see `Auditor -pending`."
        with self._lock:
            proposal = self.state.get_proposal(proposal_id)
            if proposal is None:
                self.state.prune_expired()
                self._save_state()
                return f"No pending proposal `{proposal_id}`. Use `Auditor -pending` to list."
            ...
            try:
                ...
                return f"Refused to apply `{proposal.knob}`: {exc}"
            self.state.consume_proposal(proposal_id)
            self.state.prune_expired()
            self._save_state()
        return (
    ...
    def list_pending(self) -> str:
        ...
        if proposals:
            ...
            lines.append("")
            lines.append("Apply with `Auditor -confirm <id>` before the TTL.")
        else:
    ...
    def _post_summary_to_discord(self, summary: str) -> None:
        ...
        try:
            ...
            self.discord.post_important(text, pin=False)
        except Exception:  # noqa: BLE001
...
• `Auditor -review` — run a full audit (synchronous)
