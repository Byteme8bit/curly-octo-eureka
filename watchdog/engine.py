...
class WatchdogEngine:
    ...
    def _portfolio_alerts(self, snap) -> list[tuple[str, bool]]:
        ...
        band = pnl_milestone_band(snap.pnl, baseline, self.settings.pnl_pct_threshold)

        if band != 0 and band != self.state.last_pnl_band:

            self.state.last_pnl_band = band

            pct = (snap.pnl / baseline * 100) if baseline > 0 else 0.0

            direction = "gain" if band > 0 else "loss"

            alerts.append((
                f"**Watchdog — major portfolio {direction}**\n"
                f"Portfolio ${snap.portfolio:,.2f}  |  PnL {snap.pnl:+.2f} ({pct:+.1f}%)\n"
                f"Crossed {abs(band) * self.settings.pnl_pct_threshold:.0%} milestone threshold",
                True,
            ))



        if snap.drawdown_pct >= self.settings.drawdown_warn_pct:

            if snap.drawdown_pct > self.state.last_drawdown_warn + 0.01:

                self.state.last_drawdown_warn = snap.drawdown_pct

                alerts.append((
                    f"**Watchdog — drawdown warning**\n"
                    f"Portfolio ${snap.portfolio:,.2f}  |  "
                    f"drawdown {snap.drawdown_pct:.1%} from peak\n"
                    f"Circuit breaker triggers at 15% — review holdings.",
                    False,
                ))

        elif snap.drawdown_pct < self.settings.drawdown_warn_pct * 0.5:
    ...
    def startup_message(self) -> str:
        ...
        return (
            ...
            f"Watching: session logs, `{log_name}`, receipts, paper state, diagnostics\n"
