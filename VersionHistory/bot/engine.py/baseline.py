...
class TradingEngine:
    ...
    def _handle_discord_command(self, command: str, _user_id: str) -> str:
        ...
        if command == "start":
            ...
            return "Trading **started** — bot will resume market ticks."
        ...
        if command == "stop":
            ...
            return "Trading **stopped** — no new ticks until you send `start`."
        ...
        if command == "reset":
            ...
            # Clear first (skips pinned, so the existing startup pin survives),

            # then post the new pin so the channel resets to a single pin.

            cleared, kept_pinned = self.discord.clear_recent_messages()

            self.discord.chat_log.log_event(

                f"Reset chat clear: deleted={cleared}, skipped={kept_pinned}"
    ...
    def run(self) -> None:
        ...
        self.display.startup(
            ...
            balances=self.settings.initial_balances,
