...
class PortfolioConstraints:
    ...
    def validate_intent(
        ...
        *,
        ...
        if intent.from_asset == "ETH" and size_pct <= 0:
        ...
        if intent.to_asset not in CORE_UNCAPPED and not intent.is_defensive:
            ...
            if projected > self.max_alt_allocation_pct:
                if not self.allows_alt_overweight(intent, required_edge=required_edge):
                    return ConstraintResult(
                        False,
                        size_pct,
                        (
                            f"Alt cap — {intent.to_asset} would reach {projected:.1%} "
                            f"(max {self.max_alt_allocation_pct:.0%} without strategy exception)"
                        ),
                    )
                # Allow but reduce size so we stay near cap if possible
                current = self.allocation_pct(intent.to_asset, holdings, prices)
                if current >= self.max_alt_allocation_pct:
                    return ConstraintResult(
                        False,
                        size_pct,
                        (
                            f"Alt cap — {intent.to_asset} already at {current:.1%}; "
    ...
    def trim_overweight_intents(
        ...
        for asset, qty in holdings.items():
            ...
            if alloc <= self.max_alt_allocation_pct:
                continue

            position_usd = self._position_usd(asset, holdings, prices)
            target_usd = portfolio * self.max_alt_allocation_pct
            ...
            intents.append(
                TradeIntent(
                    ...
                    reason=(
                        ...
                        f"{self.max_alt_allocation_pct:.0%} max (alt concentration)"
                    ),
                    size_pct=size_pct,
                    edge=0.0,
                    is_defensive=True,
