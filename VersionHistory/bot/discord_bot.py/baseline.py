...
_PREFIXED_PATTERN = re.compile(
    r"^(tradebot|tb|watchdog|wd)\s+-?([\w-]+)\s*$",
    re.IGNORECASE,
)
...
\u2022 `TradeBot -reset` \u2014 reset paper balances + watchdog counters; clears Discord chat
...
def _match_prefixed(text: str) -> ParsedCommand | None:
    ...
    bot = "tradebot" if bot_raw in {"tradebot", "tb"} else "watchdog"
    if bot == "tradebot":
